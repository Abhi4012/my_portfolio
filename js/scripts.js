...
// Generative AI Text Reveal
document.addEventListener('DOMContentLoaded', () => {
    const matrixBg = document.querySelector('.matrix-background');
    const generativeText = document.getElementById('generative-text');

    const textContent = "As an AI/ML Engineer, I specialize in innovative solutions, pushing the boundaries of technology through deep learning, machine learning, and generative AI. I aim to create transformative solutions that evolve with intelligence.";

    let charIndex = 0;

    function typeText() {
        if (charIndex < textContent.length) {
            generativeText.innerHTML += textContent[charIndex];
            charIndex++;
    ...
    setInterval(() => {
        ...
        factDiv.className = 'ai-fact';
        factDiv.innerText = getRandomFact();
        matrixBg.appendChild(factDiv);

        setTimeout(() => {
            matrixBg.removeChild(factDiv);
        }, 5000);
    }, 10000);

    function getRandomFact() {
        const facts = [
            "AI can generate human-like text using large language models.",
            "Generative AI creates new data such as images, sounds, and text.",
            "AI is evolving to assist in creative fields like music and art."
        ];
